...
SELECT
    ...
    COALESCE(C.customer_key, 0) AS O_CUST_WID, -- If the lookup fails, use zero
    {{ integration_key( ["O_ORDERKEY"] ) }} AS integration_id,
    HASH(
        {{ dbt_utils.star(from=source("TPC_H", "ORDERS"), except=["O_ORDERKEY"]) }},
        COALESCE(C.customer_key, 0)) AS cdc_hash_key
FROM
{{ source("TPC_H", "ORDERS") }} O
LEFT OUTER JOIN {{ ref("dim_customers") }} C on C.customer_key = O.O_CUSTKEY
